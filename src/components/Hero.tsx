		...
		try {
			...
			const apiUrl =
				...
				(currentOrigin.includes("vercel.app")
					? "" // For Vercel deployments, use a relative path by making apiUrl empty
					: "http://localhost:3001");
			...
			<div className="text-center">
				...
				{showChat && (
					<div className="mt-8 max-w-2xl mx-auto p-4 bg-background/5 border border-foreground/10 rounded-[1.5rem] flex flex-col items-center min-h-[400px] transition-all duration-300 w-[90vw] sm:w-full">
						<div
							ref={chatHistoryRef}
							className="flex-grow w-full text-left overflow-y-auto pr-2 mb-4"
						>
							{chatHistory.map((msg, index) => (
								<div
									key={index}
									className={`mb-2 text-sm ${msg.role === "user" ? "text-foreground/80" : "text-foreground"}`}
								>
									<span className="font-bold">
										{msg.role === "user" ? "you: " : "werq: "}
									</span>
									{msg.content}
								</div>
							))}
